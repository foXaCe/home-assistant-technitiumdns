...
class SmartActivityAnalyzer:
    ...
    def __init__(
        self,
        score_threshold: int = DEFAULT_ACTIVITY_SCORE_THRESHOLD,
    ...
    def analyze_device_activity(self, dns_logs: List[Dict], ip_address: str) -> Dict:
        ...
        background_queries = sum(
            1 for log in device_logs if self._is_background_query(log)
        )
        background_ratio = (
    ...
    def _filter_device_logs(self, dns_logs: List[Dict], ip_address: str) -> List[Dict]:
        ...
        for log in dns_logs:
            ...
            log_time_str = (
                log.get("timestamp") or log.get("time") or log.get("dateTime")
            )
            if not log_time_str:
                continue
            try:
    ...
    def _generate_analysis_summary(
        self, score: float, queries: int, bg_ratio: float, protocols: int
    ) -> str:
        ...
        return (
            f"{activity_level} - {queries} queries, "
...
def analyze_batch_device_activity(
    ...
    avg_score = (
        sum(result["activity_score"] for result in results.values()) / len(results)
        if results
        else 0
    )
